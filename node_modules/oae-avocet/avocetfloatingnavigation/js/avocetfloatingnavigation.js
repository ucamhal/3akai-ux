 ...
 define(['jquery', 'oae.core'], function($, oae) {
    ...
    return function(uid) {
        ...
        var renderNavigation = function() {
            oae.api.util.template().render($('#oa-avocetfloatingnavigation-nav-template', $rootel), {
                'navs': $rootel.data('nav-entries'),
                'navTitle': $rootel.data('nav-title')
            }, $('.avocetfloatingnavigation-wrapper', $rootel));
        };

        /**
         * Initialise the dynamic aspects of the navigation.
         *
         * @api private
         */
        var initialise = function() {
            // Update the navigation with an 'active' class on the nav section corresponding to the content area
            $($rootel.data('scroll-element')).scrollspy({
                'target': '#' + uid + ' ' + '.avocetfloatingnavigation',
                'offset': SCROLL_OFFSET
            });

            // Have the navigation stay fixed in the window between the top and bottom positions specified:
            $('.avocetfloatingnavigation-fix', $rootel).affix({
                'offset': {
                    'top': $('.avocetfloatingnavigation', $rootel).offset().top,
        ...
        // Defer initialisation of scrollspy & affix until the page is visible. If we initialise immediately the page isn't visible so scrollspy doesn't work correctly.
        $(window).one('widgetsready.oae', function() {
            initialise();
        });
    };
});
