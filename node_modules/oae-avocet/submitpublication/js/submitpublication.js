...
define(['jquery', 'oae.core', 'jquery.fileupload', 'jquery.iframe-transport'], function($, oae) {
    ...
    return function(uid) {
        ...
        var parts = {
            ...
            'progress': $progress,
            'upload': $upload,
            // Both the upload and success markup should be shown
            'success': $upload.add($success)
        };

        /**
         * Shows the HTML responsible for a provided part of the submission process, hides all others.
         *
         * @param  {String}   toActivate     The part which should be activated
         */
        var activatePart = function(toShow) {
            _.invoke(parts, $.fn.hide);
            if (parts[toShow]) parts[toShow].show();
        };
        ...
        var initForm = function() {
            ...
            oae.api.util.validation().validate($('form', $form), {
                ...
                'submitHandler': function(form) {
                    ...
                    oae.api.publication.createPublication(data.title, data.author, data.journal, data.funders, data.email, data.copyright, data.comment, selectedFile, $upload, {}, function(error, data) {
                        if (error) {
