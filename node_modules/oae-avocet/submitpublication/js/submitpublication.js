    ...
    return function(uid) {
        ...
        var initForm = function() {
            ...
            $(document).on('oa.publicationform.submit', function(event, data) {
                showPanels($progress);
                oae.api.publication.createPublication(data.title, data.authors, data.department, data.institution, data.journal, data.acceptanceDate, data.funders, data.email, data.useCambridgeAddendum, data.comment, $upload, selectedFile, function(error, data) {
                    ...
                    if (window.ga) {
                        ga('send', 'event', 'button', 'click', 'Publications');
                    }
                    // Reset the form
                    $(document).trigger('oa.publicationform.reset');

                    sessionStorage.setItem('avocet-last-uploaded', data.publication.ticket.externalId);
                    window.location = '/';
                });
            });
        };
