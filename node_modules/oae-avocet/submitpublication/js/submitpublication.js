...
define(['jquery', 'oae.core', 'text!/avocet/data/journals.json', 'jquery.fileupload', 'jquery.iframe-transport', 'jquery.typeahead'], function($, oae, journalsRaw) {
    ...
    return function(uid) {
        ...
        // TODO: remove once the journal feed PR is merged (@see https://github.com/CUL-DigitalServices/avocet-hilary/pull/7)
        // Parse the journals json file
        var journals = JSON.parse(journalsRaw);
        ...
        var initForm = function() {
            ...
            // Initialise autocompletetion for the journal input field
            $('#input-journal', $form).typeahead({
                'hint': true,
                'highlight': true
            }, {
                'name': 'journals',
                ...
                'source': function(query, callback) {
                    // TODO: Replace once the Journals can be retrieved through a REST endpoint (@see https://github.com/CUL-DigitalServices/avocet-hilary/pull/7)
                    var filtered = [];
                    _.each(journals, function(journal) {
                        if (filtered.length < 10 && journal.toLowerCase().indexOf(query.toLowerCase()) >= 0) {
                            filtered.push({
                                'value': journal
                            });
                        }
                ...
                'submitHandler': function(form) {
                    ...
                    oae.api.publication.createPublication(data.title, data.author, data.journal, data.funders, data.email, data.copyright, data.comment, $upload, selectedFile, function(error, data) {
                        if (error) {
                            ...
                            $progress.hide();
                            $upload.show();
                            $('#oa-submitpublication-upload-error', $upload).show();
                            return;
                        }
                        ...
                        oae.api.util.template().render($('#oa-submitpublication-success-template'), {
