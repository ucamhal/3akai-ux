/*!
 * Copyright 2014 Digital Services, University of Cambridge Licensed
 * under the Educational Community License, Version 2.0 (the
 * "License"); you may not use this file except in compliance with the
 * License. You may obtain a copy of the License at
 *
 *     http://opensource.org/licenses/ECL-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an "AS IS"
 * BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
 * or implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */

/****************
 ** Navigation **
 ****************/

/* Fix z ordering: the nav needs to be above the header and
   the other header buttons need to be above the nav. */
#oa-avocetheader .oa-nav.depth-1 {
    z-index: 1;
}

#oa-avocetheader .oa-btn-uni,
#oa-avocetheader #oa-avocetheader-authentication-container > * {
    z-index: 2;
    position: relative;
}

#oa-avocetheader {
    /* This is the offset parent for .oa-nav.depth-1 */
    position: relative;
}

#oa-avocetheader .oa-nav-items {
    /* @extend .container */
    width: 960px;
    padding-right: 15px;
    padding-left: 15px;
    margin-right: auto;
    margin-left: auto;
}

#oa-avocetheader .oa-nav {
    left: 0;
    position: absolute;
    top: 69px;
    width: 100%;
}

#oa-avocetheader .oa-nav.depth-1 > .oa-nav-items {
    /* Offset the first row of navs to make space for the Cam uni logo */
    padding-left: 255px;
}

#oa-avocetheader .oa-nav.depth-1 {
    background: none;
    top: 0;
}

#oa-avocetheader .oa-nav:not(.depth-1) > .oa-nav-items {
    height: 0;
    overflow: hidden;
    transition: height .3s, visibility 0s linear .3s;
    visibility: hidden; /* Prevent elements from being focused */
}

#oa-avocetheader .oa-nav-item.viewed > .oa-nav > .oa-nav-items,
#oa-avocetheader .oa-nav-item.active-child > .oa-nav > .oa-nav-items {
    height: 68px;
    transition: height .3s, visibility 0s linear 0s;
    visibility: visible;
}

/* Nav lower borders */
#oa-avocetheader .oa-nav-item > .oa-nav,
#oa-avocetheader .oa-nav-item > .oa-nav {
    border-bottom-style: solid;
    border-bottom-width: 0px;
    transition: border-bottom-width 0s linear .3s;
}

#oa-avocetheader .oa-nav-item.viewed > .oa-nav,
#oa-avocetheader .oa-nav-item.active-child > .oa-nav {
    border-bottom-width: 1px;
    transition: border-bottom-width 0s linear 0s;
}

#oa-avocetheader .oa-nav .oa-nav-item {
    border-bottom-style: solid;
    border-bottom-width: 5px;
    display: inline-block;
    font-size: 18px;
    margin: 0 80px 0 0;
}

#oa-avocetheader .oa-nav .oa-nav-item:last-child {
    margin-right: 0;
}

#oa-avocetheader .oa-nav .oa-nav-item a {
    display: inline-block; /* Required for padding */
    /* offset the focus halo inside the bounding box to avoid clipping due to overflow: hidden;  */
    outline-offset: -5px;
    padding: 22px 5px 14px 5px;
}

/******************
 ** Side buttons **
 ******************/
#oa-avocetheader .oa-btn-uni {
    display: inline-block;
    margin: 15px 0;
}

#oa-avocetheader .oa-btn-uni img {
    height: 38px;
    width: 178px;
}

#oa-avocetheader-authentication-container {
    padding: 9px 0;
}

#oa-avocetheader .oa-btn-upload {
    font-size: 16px;
    margin-left: 17px;
    padding: 13px 38px;
}

#oa-avocetheader .oa-btn-auth {
    font-size: 16px;
}

/*****************
 ** Logout Form **
 *****************/
 #oa-avocetheader .avocetheader-logout-form {
    display: inline;
}
