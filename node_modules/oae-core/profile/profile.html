...
<div id="profile-widget">
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane" id="profile-publications-container">
            <ul class="profile-publications oae-list">
            <ul>
        </div>
    </div>
</div>
...
<div id="profile-publications-template"><!--
    {for block in publications}
        ...
        <li class="profile-publication-year-section">
            <div class="profile-publication-year">
                <h3>${year}</h3>
            </div>
            <div></div>
        </li>
            ...
            <li class="profile-publication">
                <div class="profile-publication-main">
                    <div class="profile-publication-icon">
                        ${renderThumbnail('content', false, false)}
                        ${renderThumbnail('content', false, true)}
                        <div class="profile-publication-status"><span class="profile-publication-icon-desc"> In press</span></div>
                    </div>

                    <div class="profile-publication-type muted"><small>${publication.publicationType|encodeForHTML}</small></div>
                    <h5 class="profile-publication-title">
                        <a href="/some/where/${publication.id|encodeForHTMLAttribute}" title="${publication.displayName|encodeForHTMLAttribute}">${publication.displayName|encodeForHTML}</a>
                    </h5>
                    <div class="profile-publication-source muted">
                        ${publication.publisher|encodeForHTML}
                        {if publication.issueNumber || publication.pageBegin || publication.pageEnd}
                            {if publication.publisher} | {/if}
                            ${publication.issueNumber || '?'|encodeForHTML}: ${publication.pageBegin || '?'|encodeForHTML} - ${publication.pageEnd || '?'|encodeForHTML}
                        {/if}
                    </div>
                </div>

                <div class="profile-publication-authors">
                    <small class="muted">__MSG__AUTHORS_LABEL__</small>
                    <ul class="profile-publication-author-list">
                        {for author in publication.authors}
                            {if _.isString(author)}
                                <li>${author|encodeForHTML}</li>
                            {else}
                                <li><a href="${author.profilePath|encodeForHTMLAttribute}">${author.displayName|encodeForHTML}</a></li>
                            {/if}
                        {/for}
                    </ul>
                </div>
        ...
        {/for}
    {forelse}
    <p class="text-center">__MSG__NO_PUBLICATIONS__</p>
    {/for}
--></div>
